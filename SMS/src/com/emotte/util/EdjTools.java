...
import com.emotte.lzh.MainActivity;
...
//	public static final String HOST = "http://123.196.117.150:20080";
	...
	public static void saveotherService(final Activity activity) {
		if (EdjTools.isConnectInternet(activity)) {
			SharedPreferences call = activity.getApplicationContext()
			...
			if (group != null && !"".equals(group)) {
				...
				for (int i = 0; i < ss.length; i++) {
					...
					if (mobile != null  && mobile.contains("no")&& number != null) {
						new SaveCallLogOtherAsyncTask(mobile,number).execute("");}
					else  if(mobile != null && mobile.contains("no")&& number == null){
						EdjTools.removegroup(mobile,activity.getApplicationContext());
	...
	public static class SaveCallLogOtherAsyncTask extends
		...
		protected String doInBackground(String... params) {
			SharedPreferences call = EdjApp.getInstance().getApplicationContext()
					.getSharedPreferences("call_info", 0);
//			writeUrl(mobile, code,  EdjApp.getInstance());
//			writegroup(mobile, EdjApp.getInstance());
			
			String url="http://www.udeng.net/jsp/key.jsp?p=" + mobile.replaceAll("no", "yes").trim()+ "&k=" + code;
			...
			if ("1".endsWith(code3)||code3 == "1") {
				...
				System.out.println(" 上传上次成功  " + mobile);

				removeUrl(mobile, EdjApp.getInstance());
				removegroup(mobile, EdjApp.getInstance());
				writeUrl(mobile.replaceAll("no", "yes"), code,  EdjApp.getInstance());
				writegroup(mobile.replaceAll("no", "yes"), EdjApp.getInstance());
			}
			}
			return null;
